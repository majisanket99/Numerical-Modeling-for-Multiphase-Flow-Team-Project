...
import cv2,cv #Uses both of cv and cv2
...
###                pygame.draw.circle(screen,PURPLE,tips,5) #Draws the vertices purple
        ...
        try:
		    ...
		    #dY = 0
                ...
                #xminpx = 481. # use "$ xdotool getmouselocation"
                #xmaxpx = 942.
                #yminpx = 161.
                #ymaxpx = 624.
                #relx = (mouseX-xminpx)/(xmaxpx-xminpx)
                #rely = 1.0 - (mouseY-yminpx)/(ymaxpx-yminpx)
                #relx = min( max( relx, 0. ), 1. )
                #rely = min( max( rely, 0. ), 1. )
                #print(repr(relx) + ' ' + repr(rely) + '\n')
                
                f = open('../results/targetPos.dat', 'r+')
                #print(relx, rely, file=f)
                ##print(dX, dY, file=f)
                print(str.format("{0:.2f}", 1.-centroidX/float(xSize)), str.format("{0:.2f}",1.-centroidY/float(ySize)), file=f)
                f.close()
